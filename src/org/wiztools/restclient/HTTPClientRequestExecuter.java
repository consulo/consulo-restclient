...
public class HTTPClientRequestExecuter implements RequestExecuter {
	...
	public void execute(Request request, View... views) {
		...
		//TODO [VISTALL] Set request timeout (default 1 minute--60000 milliseconds)
		...
		try {
			...
			// Get request headers
			MultiMap<String, String> header_data = request.getHeaders();
			...
			if(entity != null) {
				if(request.isIgnoreResponseBody()) {
					...
					try{
						byte[] responseBody = StreamUtil.loadFromStream(is);
